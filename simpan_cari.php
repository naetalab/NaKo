...
<table border="0" cellspacing="1" cellpadding="0">
  <tr>
    ...
    <td id="namaField">Jumlah Kredit</td>
    <td id="namaField">Saldo</td>
    <td id="namaField">Tgl Awal Masuk</td>
    <td id="namaField">Keterangan</td>
    <td id="namaField" colspan="2"> Menu</td>
  </tr>
  
<?php 
		$cari="SELECT * FROM simpan WHERE $_POST[pilih] LIKE '%$_POST[tcari]%'";
		$sum="SELECT SUM(simpan_sisa) AS total FROM simpan WHERE $_POST[pilih] LIKE '%$_POST[tcari]%'";
		$qcari=mysql_query($cari);
		$no=1;
		$total_sisa=0;
		while($data=mysql_fetch_array($qcari))
		{
			if ($warna==$warna1){
				$warna=$warna2;
			}
  ...
  <tr bgcolor=<?php echo $warna; ?>>
    ...
    <td>
    <a href="#" onclick="javascript:wincal=window.open('simpan_rinci.php?id=<?php echo $data['jual_id']; ?>','Lihat Data','width=790,height=400,scrollbars=1');">
    <div id="tombol">rincian</div></a>
    </td>
    <td><?php echo "<a href=index.php?halaman=simpan_bayar&id=$data[jual_id]><div id=tombol>Bayar</div></a>"; ?></td>
  </tr>
  <?php
	}
  ?>
  <tr>
    ...
    <?php
		$PAwl="SELECT SUM(simpan_awal) AS PAwl FROM simpan WHERE $_POST[pilih] LIKE '%$_POST[tcari]%'";
    ...
    <td style="color:#FFF; background-color:#333; border:none" align="right">
    <?php
		$TjmlB="SELECT SUM(jml_bayar) AS TjmlB FROM simpan WHERE $_POST[pilih] LIKE '%$_POST[tcari]%'";
...
<a href="index.php?halaman=simpan"><div id="tombolAdd">kembali</div></a>
</body>
</html>
<?php
	}
